"""
BitNet model implementation with layer skipping.
"""
...
from .early_exit import validate_tensor, compute_early_exit_loss
...
class BitNetModel(nn.Module):
    """
    BitNet model with layer skipping capabilities and early exit loss.
    
    This model implements a transformer architecture with layer skipping and
    ...
    def __init__(self, config: DefaultConfig):
        super().__init__()
        self.config = config
        logger.info(f"Initializing BitNet model with config: {config}")
        ...
        logger.info(f"Initialized {config.num_hidden_layers} transformer layers")
            ...
            if labels is not None:
                ...
                if bool(getattr(self, 'training', False)) and getattr(self.config, 'lambda_q', 0) > 0:
                    ...
                    for module in self.modules():
                        if hasattr(module, 'weight') and isinstance(module, (nn.Linear, BitLinear)) and not isinstance(module, nn.Embedding):
                            if hasattr(module, 'weight_scale'):
                                # BitLinear: MSE between original and quantized weights
                                w = module.weight
