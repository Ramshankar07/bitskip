...
class BitNetAttention2(nn.Module):
    """
    Multi-head attention using H-BitLinear layers with RoPE.
    
    Args:
        hidden_size: Hidden size of the model (must be power of 2)
    ...
    def __init__(
        ...
        # Ensure hidden_size is a power of 2 for H-BitLinear
        if not (hidden_size & (hidden_size - 1) == 0):
            raise ValueError(f"hidden_size ({hidden_size}) must be a power of 2 for H-BitLinear")
        
        # Log dimensions for debugging
        logger = logging.getLogger(__name__)
        logger.info(f"BitNetAttention2 initialized with hidden_size={hidden_size}, num_heads={num_heads}, head_dim={self.head_dim}, activation_bits={self.activation_bits}")
        
        # QKV projections using H-BitLinear
        self.q_proj = HBitLinear(hidden_size, hidden_size, bias=False)
        self.k_proj = HBitLinear(hidden_size, hidden_size, bias=False)
        self.v_proj = HBitLinear(hidden_size, hidden_size, bias=False)
        
        # Output projection using H-BitLinear
        self.o_proj = HBitLinear(hidden_size, hidden_size, bias=False)
    ...
    def forward(
        ...
        # Project queries, keys, values with normalized input using H-BitLinear
        q = self.q_proj(normed, bits=self.activation_bits)
        k = self.k_proj(normed, bits=self.activation_bits)
        v = self.v_proj(normed, bits=self.activation_bits)
        ...
        # Compute attention
        with torch.autograd.profiler.record_function("Attention2.compute"):
        ...
        # Final projection with H-BitLinear
        attn_output = self.o_proj(attn_output, bits=self.activation_bits)
