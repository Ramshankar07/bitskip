#!/usr/bin/env python3
"""
No Dropout BitNet Training Script - Pure Native BitNet (Control)

This script implements the control model for ablation study:
1. Native BitNet architecture
2. NO layer dropout (control group)
3. BitLinear layers in feed forward
4. Standard training without any skipping

This serves as the baseline to measure the impact of layer dropout.
"""
...
class NoDropoutBitNetConfig(DefaultConfig):
    """No Dropout configuration for ablation study - Pure Native BitNet (Control)."""
    
    def __init__(self, **kwargs):
        # Extract no dropout parameters (all disabled)
        no_dropout_params = {
            'use_layer_skipping': kwargs.pop('use_layer_skipping', False),  # DISABLED
            'skip_probability': kwargs.pop('skip_probability', 0.0),  # NO SKIPPING
            'min_layers_to_keep': kwargs.pop('min_layers_to_keep', 30),  # KEEP ALL LAYERS
            'use_early_exit': kwargs.pop('use_early_exit', False),  # DISABLED
            'early_exit_threshold': kwargs.pop('early_exit_threshold', 1.0),  # NO EARLY EXIT
        }
        
        # Call parent constructor with remaining kwargs
        super().__init__(**kwargs)
        
        # Set no dropout parameters as attributes
        for key, value in no_dropout_params.items():
...
def setup_logging(log_dir: str):
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(log_dir, 'no_dropout_training.log')),
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='No Dropout BitNet training - Pure Native BitNet (Control)'
    )
    ...
    parser.add_argument('--output_dir', type=str, default='./output-no-dropout',
...
def main():
    """Main training function for no dropout model (control)."""
    args = parse_args()
    ...
    logger.info("=" * 80)
    logger.info("NO DROPOUT BITNET TRAINING - PURE NATIVE BITNET (CONTROL)")
    logger.info("=" * 80)
    logger.info("Ablation Study: Script 0/4 (Control Group)")
    logger.info("Features: Native BitNet + BitLinear + NO Layer Dropout")
    logger.info("Purpose: Baseline to measure layer dropout impact")
    logger.info("=" * 80)
    ...
    # Create no dropout configuration (control)
    # Only override DefaultConfig values if command line arguments are provided
    config_kwargs = {
        "dataset_name": "HuggingFaceFW/fineweb-edu",
        "subset": "sample-10BT",
        "vocab_size": actual_vocab_size,
        # NO DROPOUT FEATURES (control group)
        "use_layer_skipping": False,  # DISABLED
        "skip_probability": 0.0,  # NO SKIPPING
        "use_early_exit": False,  # DISABLED
        "early_exit_threshold": 1.0,  # NO EARLY EXIT
    ...
    if args.num_layers is not None:
        ...
        config_kwargs["min_layers_to_keep"] = args.num_layers  # KEEP ALL LAYERS
    ...
    config = NoDropoutBitNetConfig(**config_kwargs)
    
    # Initialize no dropout model (Native BitNet - control)
    logger.info("Initializing no dropout BitNet model (Native BitNet - Control)...")
    
    # Log all configuration parameters
    logger.info("=" * 60)
    logger.info("CONFIGURATION PARAMETERS (CONTROL GROUP)")
    logger.info("=" * 60)
    logger.info(f"Model Architecture:")
    logger.info(f"  - Model Type: Native BitNet (model1)")
    logger.info(f"  - Hidden Size: {config.hidden_size} (from DefaultConfig)")
    logger.info(f"  - Number of Layers: {config.num_hidden_layers} (from DefaultConfig)")
    logger.info(f"  - Number of Heads: {config.num_attention_heads} (from DefaultConfig)")
    logger.info(f"  - Max Sequence Length: {config.max_position_embeddings} (from DefaultConfig)")
    logger.info(f"  - Vocabulary Size: {config.vocab_size}")
    logger.info(f"")
    logger.info(f"Training Parameters:")
    logger.info(f"  - Learning Rate: {config.learning_rate} (from DefaultConfig)")
    logger.info(f"  - Batch Size: {config.batch_size} (from DefaultConfig)")
    logger.info(f"  - Number of Steps: {args.num_steps}")
    logger.info(f"  - Output Directory: {config.output_dir}")
    logger.info(f"")
    logger.info(f"Control Group Features:")
    logger.info(f"  - Layer Skipping: {config.use_layer_skipping} (DISABLED)")
    logger.info(f"  - Skip Probability: {config.skip_probability} (NO SKIPPING)")
    logger.info(f"  - Min Layers to Keep: {config.min_layers_to_keep} (ALL LAYERS)")
    ...
    logger.info("=" * 60)
    logger.info("NOTE: Using DefaultConfig values unless overridden by command line arguments")
    logger.info("=" * 60)
    ...
    if os.path.exists(args.output_dir):
        ...
        checkpoint_files = [f for f in os.listdir(args.output_dir) if f.startswith('no_dropout_checkpoint_step_') and f.endswith('.pt')]
        if checkpoint_files:
    ...
    # Training loop (standard - no dropout)
    logger.info("Starting no dropout training (control group)...")
    ...
    for step in range(args.num_steps):
        try:
            ...
            if global_step % args.logging_steps == 0:
                ...
                logger.info(f"Control Group: No layer dropout, no early exit")
            ...
            if global_step % args.save_steps == 0:
                checkpoint_path = os.path.join(
                    args.output_dir,
                    f'no_dropout_checkpoint_step_{global_step}.pt'
                )
                torch.save({
                ...
                logger.info(f"Saved no dropout checkpoint to {checkpoint_path}")
    ...
    try:
        ...
        plt.ylabel('Loss')
        plt.title('No Dropout BitNet Training Loss (Control Group)')
        plt.legend()
        ...
        # Save the plot
        plot_path = os.path.join(args.output_dir, 'no_dropout_training_loss.png')
    ...
    # Save final model
    final_model_path = os.path.join(args.output_dir, 'no_dropout_final_model.pt')
    torch.save({
        'model_state_dict': model.state_dict(),
        'config': config,
    ...
    logger.info("=" * 80)
    logger.info("NO DROPOUT TRAINING COMPLETED SUCCESSFULLY! (CONTROL GROUP)")
    logger.info("=" * 80)
    logger.info("Features: Native BitNet + BitLinear + NO Layer Dropout")
    logger.info("Purpose: Baseline control group for ablation study")
