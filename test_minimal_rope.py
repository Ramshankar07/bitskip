#!/usr/bin/env python3
"""
Minimal test to verify RoPE generation works correctly.
"""
...
def test_rope_generation_minimal():
    """Test RoPE generation with the exact same logic as in our wrapper."""
    logger.info("Testing minimal RoPE generation...")
    ...
    try:
        ...
        inv_freq = 1.0 / (rope_theta ** (torch.arange(0, hidden_size, 2, dtype=torch.float32, device=position_ids.device) / hidden_size))
        ...
        logger.info("✅ Minimal RoPE generation test passed!")
        return True
        
    except Exception as e:
        logger.error(f"❌ Minimal RoPE generation test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    try:
        success = test_rope_generation_minimal()
        if success:
            logger.info("🎉 Minimal RoPE test passed!")
        else:
            logger.error("❌ Minimal RoPE test failed!")
    except Exception as e:
