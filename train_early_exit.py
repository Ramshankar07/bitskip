#!/usr/bin/env python3
"""
Early Exit BitNet Training Script - Baseline + Early Exit

This script implements the second model for ablation study:
...
class EarlyExitBitNetConfig(DefaultConfig):
    """Early Exit configuration for ablation study - Baseline + Early Exit."""
    
    def __init__(self, **kwargs):
        # Extract early exit parameters
        early_exit_params = {
        ...
        # Set early exit parameters as attributes
        for key, value in early_exit_params.items():
...
def setup_logging(log_dir: str):
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(log_dir, 'early_exit_training.log')),
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Early Exit BitNet training - Baseline + Early Exit'
    )
    ...
    parser.add_argument('--output_dir', type=str, default='./output-early-exit',
...
def main():
    """Main training function for early exit model."""
    args = parse_args()
    ...
    logger.info("=" * 80)
    logger.info("EARLY EXIT BITNET TRAINING - BASELINE + EARLY EXIT")
    logger.info("=" * 80)
    logger.info("Ablation Study: Script 2/3")
    logger.info("Features: Native BitNet + Layer Dropout + BitLinear + Early Exit")
    logger.info("=" * 80)
    ...
    # Create early exit configuration
    ...
    config = EarlyExitBitNetConfig(**config_kwargs)
    
    # Initialize early exit model (Native BitNet)
    logger.info("Initializing early exit BitNet model (Native BitNet)...")
    
    # Log all configuration parameters
    logger.info("=" * 60)
    logger.info("CONFIGURATION PARAMETERS (EARLY EXIT)")
    logger.info("=" * 60)
    ...
    logger.info(f"")
    logger.info(f"Early Exit Features:")
    ...
    if os.path.exists(args.output_dir):
        ...
        checkpoint_files = [f for f in os.listdir(args.output_dir) if f.startswith('early_exit_checkpoint_step_') and f.endswith('.pt')]
        if checkpoint_files:
    ...
    logger.info("Starting early exit training...")
        ...
        try:
            ...
            if global_step % args.save_steps == 0:
                checkpoint_path = os.path.join(
                    args.output_dir,
                    f'early_exit_checkpoint_step_{global_step}.pt'
                )
                torch.save({
                ...
                logger.info(f"Saved early exit checkpoint to {checkpoint_path}")
    ...
    try:
        ...
        plt.ylabel('Loss')
        plt.title('Early Exit BitNet Training Loss (Layer Dropout + Early Exit)')
        plt.legend()
        ...
        # Save the plot
        plot_path = os.path.join(args.output_dir, 'early_exit_training_loss.png')
    ...
    # Save final model
    final_model_path = os.path.join(args.output_dir, 'early_exit_final_model.pt')
    torch.save({
        'model_state_dict': model.state_dict(),
        'config': config,
    ...
    logger.info("=" * 80)
    logger.info("EARLY EXIT TRAINING COMPLETED SUCCESSFULLY!")
    logger.info("=" * 80)
    logger.info("Features: Native BitNet + Layer Dropout + BitLinear + Early Exit")
