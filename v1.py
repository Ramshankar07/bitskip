...
# Use the exact same imports as the template
...
# ---------------- Optimized All2All implementation ----------------
class OptimizedAllToAll:
    ...
    def __init__(self, cfg: MoEConfig, rank: int, world_size: int):
        ...
        # Pre-allocate persistent buffers to avoid repeated allocation
    ...
    def _init_persistent_buffers(self):
        ...
        max_total_tokens = cfg.max_num_tokens * self.world_size * cfg.experts_per_token
        
        # Persistent count tensors
        self.send_counts_cache = torch.empty(
            self.world_size, dtype=torch.long, device=device
        )
        self.recv_counts_cache = torch.empty(
            self.world_size, dtype=torch.long, device=device
        )
        
        # Expert buffers
        self.expert_num_tokens_cache = torch.zeros(
            self.num_local_experts, dtype=torch.int32, device=device
        )

    def _vectorized_token_mapping(self, indices: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Optimized token mapping using vectorized operations"""
        device = indices.device
        
        # Flatten indices for vectorized processing
        flat_indices = indices.view(-1)  # [num_tokens * experts_per_token]
        
        # Create token and expert position arrays
        num_tokens, experts_per_token = indices.shape
        token_ids = torch.arange(num_tokens, device=device).repeat_interleave(experts_per_token)
        expert_positions = torch.arange(experts_per_token, device=device).repeat(num_tokens)
        
        # Vectorized rank calculation
        dst_ranks = flat_indices // self.num_local_experts
        
        # Create metadata tensor in one go
        meta_tensor = torch.stack([
            flat_indices,  # global expert
            torch.full_like(flat_indices, self.rank),  # src_rank
            token_ids,  # src_token
            expert_positions,  # src_k
            torch.zeros_like(flat_indices)  # padding
        ], dim=1)
        
        # Count tokens per rank using bincount
        send_counts = torch.bincount(dst_ranks, minlength=self.world_size)
        
        # Sort by destination rank for efficient packing
        sort_indices = torch.argsort(dst_ranks)
        
        return sort_indices, send_counts, meta_tensor[sort_indices]

    def dispatch(self, dp_x: torch.Tensor, indices: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Optimized dispatch with vectorized operations and memory reuse"""
        device = dp_x.device
        cfg = self.cfg
        
        # Reset expert token counts
        self.expert_num_tokens_cache.zero_()
        
        # Vectorized token mapping
        sort_indices, send_counts, sorted_meta = self._vectorized_token_mapping(indices)
        
        # Copy counts to cache
        self.send_counts_cache[:] = send_counts
        
        # Get receive counts via all-to-all
        dist.all_to_all_single(self.recv_counts_cache, self.send_counts_cache)
        
        # Prepare send buffer using advanced indexing
        token_indices = sorted_meta[:, 2]  # src_token column
        total_send = len(sort_indices)
        
        # Create send buffer by gathering input tokens
        if total_send > 0:
            send_buf = dp_x[token_indices]
        else:
            send_buf = torch.empty((0, cfg.hidden_dim), dtype=cfg.in_dtype, device=device)
        
        # Prepare receive buffer
        total_recv = int(self.recv_counts_cache.sum().item())
        if total_recv > 0:
            ...
            recv_buf = torch.empty((0, cfg.hidden_dim), dtype=cfg.in_dtype, device=device)
            recv_meta = torch.empty((0, self.META_DIM), dtype=torch.int32, device=device)
        ...
        if total_recv > 0 and total_send > 0:
            ...
            dist.all_to_all_single(
                recv_buf,
                send_buf,
                output_split_sizes=self.recv_counts_cache.tolist(),
                input_split_sizes=self.send_counts_cache.tolist(),
            )
            ...
            dist.all_to_all_single(
                recv_meta.view(-1),
                sorted_meta.view(-1),
                output_split_sizes=[c * self.META_DIM for c in self.recv_counts_cache.tolist()],
                input_split_sizes=[c * self.META_DIM for c in self.send_counts_cache.tolist()],
            )
        ...
        # Vectorized expert assignment
        if total_recv > 0:
            global_expert_ids = recv_meta[:, 0]
            local_expert_ids = global_expert_ids % self.num_local_experts
            ...
            for local_eid in range(self.num_local_experts):
                ...
                if mask.any():
                    expert_tokens = recv_buf[mask]
                    expert_metas = recv_meta[mask]
                    num_tokens = len(expert_tokens)
                    
                    # Assign to expert buffers
                    expert_x[local_eid, :num_tokens] = expert_tokens
                    expert_meta[local_eid, :num_tokens] = expert_metas
                    self.expert_num_tokens_cache[local_eid] = num_tokens
        
        return self.expert_num_tokens_cache.clone(), expert_x, expert_meta
    ...
    def combine(
        ...
        """Optimized combine with vectorized operations"""
        device = out_tokens.device
        cfg = self.cfg
        
        # Collect all tokens and metadata to send back
        all_tokens = []
        all_meta = []
        ...
        for local_eid in range(self.num_local_experts):
            ...
            if num_tokens > 0:
                all_tokens.append(expert_y[local_eid, :num_tokens])
                all_meta.append(expert_meta[local_eid, :num_tokens])
        
        if not all_tokens:
            # No tokens to send back
            ...
            send_buf = torch.cat(all_tokens, dim=0)
            send_meta = torch.cat(all_meta, dim=0)
            total_send = len(send_buf)
        
        # Vectorized send count calculation
        if total_send > 0:
            dst_ranks = send_meta[:, 1]  # src_rank column
            send_counts = torch.bincount(dst_ranks, minlength=self.world_size)
        else:
        ...
        # Sort send data by destination rank for efficient communication
        if total_send > 0:
            sort_indices = torch.argsort(dst_ranks)
            send_buf = send_buf[sort_indices]
            send_meta = send_meta[sort_indices]
        ...
        if total_recv > 0:
            ...
            dist.all_to_all_single(
                ...
                input_split_sizes=[c * self.META_DIM for c in send_counts.tolist()],
            )
            
            # Vectorized weighted sum using advanced indexing
            src_tokens = recv_meta[:, 2]
            src_ks = recv_meta[:, 3]
            
            # Get weights for all tokens at once
            token_weights = weights[src_tokens, src_ks].to(torch.float32)
            
            # Convert received data to float32 and apply weights
            weighted_data = recv_buf.to(torch.float32) * token_weights.unsqueeze(-1)
            
            # Scatter-add to output tokens
            out_tokens.index_add_(0, src_tokens, weighted_data)
...
def custom_kernel(data: input_t) -> output_t:
    """Competition submission - optimized All-to-All kernel"""
    cfg, rank_data, rank, world_size = data

    # Use optimized implementation
    ata = OptimizedAllToAll(cfg, rank, world_size)

    # Dispatch phase - distribute tokens to experts
    expert_num, expert_x, expert_meta = ata.dispatch(rank_data.x, rank_data.indices)
    
    # Expert computation (simplified as multiplication)
    ...
    y = torch.zeros(
        cfg.max_num_tokens,
        cfg.hidden_dim,
        dtype=torch.float32,  # Use float32 for accumulation to avoid precision loss
        device=rank_data.x.device,
    )

    # Combine phase - gather results back
    ata.combine(y, rank_data.weights, expert_meta, expert_y, expert_num)
    
    # Convert back to output dtype and return only valid tokens
    return y[: rank_data.num_tokens].to(cfg.out_dtype)

def ref_kernel(data: input_t) -> output_t:
    """Reference implementation using original PyTorch All-to-All"""
    cfg, rank_data, rank, world_size = data
...
def check_implementation(data: input_t, output: output_t):
    ...
    if not res:
        return False, f"Output values mismatch"
    
    return True, ""

# ---------------- Original PyTorch All2All for reference ----------------
class PyTorchAllToAll:
    ...
    def dispatch(self, dp_x: torch.Tensor, indices: torch.Tensor):
        ...
        send_buf = torch.cat([dp_x[idx_list] for idx_list in token_map], dim=0) if any(token_map) else torch.empty((0, cfg.hidden_dim), dtype=cfg.in_dtype, device=device)
        ...
        ).view(-1, self.META_DIM) if any(meta_map) else torch.empty((0, self.META_DIM), dtype=torch.int32, device=device)
        recv_meta = torch.empty(
            total_recv, self.META_DIM, dtype=torch.int32, device=device
        )
